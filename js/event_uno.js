...
const EventUno = (() => {
  ...
  const TILE         = 64;
  const GRID_COLS    = 32; // خريطة أطول لسباق 50 لاعباً
  const GRID_ROWS    = 14;
  const ROUND_TIME   = 30;
  ...
  let _freeCam = { x:0, y:0, active:false }; // كاميرا المتفرج
  ...
  function _buildCards() {
    ...
    // مساحة السباق بالكامل لتوزيع البطاقات بشكل عشوائي
    const zoneStartX = SAFE_COLS*TILE;
    const zoneEndX = GRID_END*TILE - TILE;

    // إقصاء تصاعدي: كل جولة يقل عدد البطاقات الصحيحة
    const correctCount = Math.max(1, alive - (_roundNum * 2));
    ...
    // فوضى البطاقات الخاطئة (عشوائية تماماً)
    const fakeCount = alive * 2; 
    for(let i=0; i<fakeCount; i++) {
      let rc = COLORS[Math.floor(Math.random()*COLORS.length)];
      let rn = NUMBERS[Math.floor(Math.random()*NUMBERS.length)];
      if(rc === _targetCard.color && rn === _targetCard.number) rc = _fakeSimilarColor(rc); // منع تطابق الصدفة
  ...
  function _buildCapsules() {
    ...
    // بناء 4 كبسولات للألوان و 4 كبسولات للأرقام في منطقة الأمان
    let startY = TILE;
    COLORS.forEach((color, i) => {
      _capsules.push({ type: 'color', value: color, x: cx + TILE*0.5, y: startY + (i*TILE*3), w: TILE*1.5, h: TILE*1.5, occupantsCount: 0 });
    });
    ...
    nums.sort(()=>Math.random()-0.5).forEach((num, i) => {
      _capsules.push({ type: 'number', value: num, x: cx + TILE*2.5, y: startY + (i*TILE*3), w: TILE*1.5, h: TILE*1.5, occupantsCount: 0 });
    });
  }
  ...
  function _buildBlockers() {
    ...
    for(let i=0; i<count; i++) {
      _blockers.push({
        ...
        w: TILE, h: TILE/3,
        visible: Math.random()<0.5, timer: Utils.randFloat(1.2, 3.0), period: Utils.randFloat(1.2, 3.0),
      });
    }
  }
  ...
  function _updateSweep(delta) {
    _sweepT += delta; const dur = 2.5; const prog = Math.min(_sweepT/dur, 1);
  ...
  function _updateRunning(delta) {
    ...
    if(_actionCard === 'skip' && !_skipApplied && _roundTimer <= 15) {
      _skipApplied = true; _roundTimer = Math.max(0, _roundTimer-5);
    ...
    // كاميرا حرة للمتفرج
    if(_me.spectating || !_me.alive || _me.falling) {
       _freeCam.active = true;
      ...
      _checkPickup(); _checkFragileMe();
    }

    if(_me.falling && _me.alive && !_me.spectating) {
      _me.fallT += delta;
      if(_me.fallT > 1.0) { 
        _me.falling = false; _me.fallT = 0; 
        _loseHeart(_myId); 
  ...
  function _updateMeMovement(delta) {
    const jx = _me.jdx || Joystick.getDx(), jy = _me.jdy || Joystick.getDy();
    ...
    if(_me.moving) {
      ...
      _me.ft += delta; if(_me.ft >= 0.13){ _me.ft=0; _me.frame=(_me.frame+1)%3; }
  ...
  function _updateBots(delta) {
    for(const p of _players) {
      ...
      const spd = (p.botLevel==='hard'?170 : p.botLevel==='medium'?130 : 90) * delta;
      ...
      if(!p.card) {
        ...
        if(nearest) {
          const dx=nearest.x-p.x, dy=nearest.y-p.y; const dm=Math.sqrt(dx*dx+dy*dy);
          if(dm>8) {
            let nx=p.x+dx/dm*spd, ny=p.y+dy/dm*spd;
            if(_canMove(nx, p.y)) p.x = Utils.clamp(nx, 0, WORLD_W);
            if(_canMove(p.x, ny)) p.y = Utils.clamp(ny, 0, WORLD_H);
          } else { nearest.taken=true; p.card=nearest; }
        }
      } else {
        // العودة للكبسولة المناسبة قبل نفاد الوقت
        let target=null;
        for(const cap of _capsules) {
          if((cap.type==='color' && p.card.color===cap.value) || (cap.type==='number' && p.card.number===cap.value)){
            target=cap; break;
          }
        }
        if(target) {
          const dx=(target.x+target.w/2)-p.x, dy=(target.y+target.h/2)-p.y; const dm=Math.sqrt(dx*dx+dy*dy);
          if(dm>12) {
            let nx=p.x+dx/dm*spd, ny=p.y+dy/dm*spd;
            if(_canMove(nx, p.y)) p.x = Utils.clamp(nx, 0, WORLD_W);
            if(_canMove(p.x, ny)) p.y = Utils.clamp(ny, 0, WORLD_H);
  ...
  function _checkFragileMe() {
    for(const f of _fragiles) {
      ...
      if(on && f.state === 'normal') {
        f.state = 'cracked'; f.crackT = 0.8; // فاصل زمني للنجاة
      }
    }
  }
  ...
  function _onTileFallen(tile) {
    const list = [..._players];
    if(_me.alive && !_me.spectating) list.push(_me);
  ...
  function _loseHeart(id) {
    if(id === _myId) {
      const me = _getMyPlayer(); if(!me) return;
      me.hearts--;
      if(me.hearts <= 0) { me.alive = false; me.spectating = true; UI.showToast('💀 تم إقصاؤك!', 2500); }
      else { UI.showToast(`💔 خسر قلباً (${me.hearts}/3)`, 1500); }
      _me.hearts = me.hearts;
      _me.alive = me.alive;
      _me.spectating = me.spectating;
    } else {
  ...
  function _endRound() {
    ...
    if(_me.alive && !_me.spectating) allEntities.push(Object.assign(_me, {id: _myId}));

    // احتساب النجاة بناءً على التواجد الفيزيائي الفعلي داخل الكبسولة وبطاقة اللاعب
    for(const p of allEntities) {
      ...
      const inCap = _capsules.find(c => p.x > c.x && p.x < c.x+c.w && p.y > c.y && p.y < c.y+c.h);
      
      if(inCap) {
        const matchesType = (inCap.type === 'color' && inCap.value === card.color) || 
                            (inCap.type === 'number' && inCap.value === card.number);
        
        if(matchesType && inCap.occupantsCount < 5) {
          inCap.occupantsCount++;
          if(p.id === _myId) UI.showToast('✅ نجوت!', 1000);
        } else {
          if(p.id === _myId) UI.showToast('❌ الكبسولة ممتلئة أو بطاقة خاطئة!', 1500);
          _loseHeart(p.id);
        }
      } else {
        _loseHeart(p.id);
    ...
    if(alive.length <= 1) {
      _phase = 'gameover';
      if(alive.length === 0) UI.showToast('🤝 لا يوجد ناجون!', 4000);
  ...
  function push(dir) {
    ...
    _me.pushCharge = 0;
    const off = { up:{dx:0, dy:-TILE*1.5}, down:{dx:0, dy:TILE*1.5}, left:{dx:-TILE*1.5, dy:0}, right:{dx:TILE*1.5, dy:0} };
  ...
  function _drawFragiles(ctx) {
    for(const f of _fragiles) {
      if(f.state === 'fallen') {
        ctx.fillStyle = 'rgba(0,0,0,0.85)'; ctx.fillRect(f.x, f.y, f.w, f.h);
        ctx.strokeStyle = '#333'; ctx.lineWidth = 1; ctx.strokeRect(f.x, f.y, f.w, f.h);
        continue;
      }
      if(f.state === 'normal') {
        ctx.fillStyle = 'rgba(255,140,0,0.15)'; ctx.fillRect(f.x, f.y, f.w, f.h);
        ctx.strokeStyle = 'rgba(255,140,0,0.4)'; ctx.lineWidth = 1; ctx.strokeRect(f.x+2, f.y+2, f.w-4, f.h-4);
      } else {
        ctx.fillStyle = 'rgba(255,50,0,0.4)'; ctx.fillRect(f.x, f.y, f.w, f.h);
        ctx.strokeStyle = 'rgba(255,0,0,0.9)'; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.moveTo(f.x+f.w*.3, f.y); ctx.lineTo(f.x+f.w*.5, f.y+f.h*.5); ctx.lineTo(f.x+f.w*.8, f.y+f.h); ctx.stroke();
        ctx.beginPath(); ctx.moveTo(f.x, f.y+f.h*.4); ctx.lineTo(f.x+f.w*.5, f.y+f.h*.5); ctx.lineTo(f.x+f.w, f.y+f.h*.7); ctx.stroke();
      }
    }
  }
  ...
  function _drawCapsules(ctx) {
    ...
    for(const cap of _capsules) {
      const cColor = cap.type === 'color' ? (COLOR_HEX[cap.value]||'#fff') : '#555';
      const pulse = 0.6 + Math.sin(t*3)*0.4;
      
      ctx.fillStyle = cColor + '33'; ctx.fillRect(cap.x, cap.y, cap.w, cap.h);
      ctx.strokeStyle = cColor; ctx.lineWidth = 3; ctx.strokeRect(cap.x, cap.y, cap.w, cap.h);
      
      ctx.font = '10px "Press Start 2P"'; ctx.textAlign = 'center'; ctx.fillStyle = '#fff';
      if(cap.type === 'color') ctx.fillText(cap.value.toUpperCase(), cap.x+cap.w/2, cap.y+16);
      else ctx.fillText('NUM: '+cap.value, cap.x+cap.w/2, cap.y+16);

      ctx.font = '8px sans-serif'; ctx.fillStyle = '#aaa';
      ctx.fillText(`Max 5`, cap.x+cap.w/2, cap.y+cap.h-8);
    }
  }
  ...
  function _drawMe(ctx) {
    ...
    if(!me && !_me.alive && !_me.spectating && !_me.falling) return;
  ...
  function _drawDarkness(ctx, cw, ch) {
    const focusX = _freeCam.active ? _freeCam.x - _camX + cw/2 : _me.x - _camX;
    const focusY = _freeCam.active ? _freeCam.y - _camY + ch/2 : _me.y - _camY;
    const gr = ctx.createRadialGradient(focusX, focusY, TILE*.4, focusX, focusY, TILE*2.8);
  ...
  function _drawHUD(ctx, cw, ch) {
    ...
    if(!_me.spectating) {
       ...
       Utils.drawPixelText(ctx, 'SPECTATOR CAMERA', 14, ch-30, {font:'5px "Press Start 2P"', color:'#00aaff', align:'left'});
    }
  ...
  function _buildUI() {
    ...
    for(const b of btns) {
      const btn = document.createElement('button'); btn.id = b.id; btn.innerHTML = b.icon;
      btn.style.cssText = s(b.bg) + `;grid-column:${b.col};grid-row:${b.row}`;
      btn.addEventListener('touchstart', e => { e.preventDefault(); b.dir==='dodge'?dodge():b.dir==='steal'?steal():push(b.dir); }, {passive:false});
      btn.addEventListener('mousedown', () => { b.dir==='dodge'?dodge():b.dir==='steal'?steal():push(b.dir); });
  ...
  function _getAlivePlayers(){ const list=_players.filter(p=>p.alive); const me=_getMyPlayer(); if(me&&me.alive) list.push(me); return list; }

  function exit(){
    _active=false; _removeUI(); _showWorldUI();
    if(typeof EventManager!=='undefined') EventManager.startTransitionOut(()=>{ UI.showToast('عدت إلى العالم 🌍',2000); });
  }
